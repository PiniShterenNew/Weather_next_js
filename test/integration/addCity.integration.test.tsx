...
describe('Add City Integration Flow', () => {
  ...
  it('should search for a city in Hebrew, select it, and add it to the store', async () => {
    ...
    expect(screen.getByText(/הוספת עיר/i)).toBeInTheDocument();
  ...
  it('should add a city from popular cities tab', async () => {
    ...
    // 2. Expand a continent section if needed (assuming Europe is collapsed)
    const europeSection = screen.getByRole('button', { name: /אירופה/i });
    await user.click(europeSection);

    // 3. Find a city in the popular list (e.g., Paris)
    // Note: This depends on your actual popular cities data
    // We're assuming Paris is in the list
